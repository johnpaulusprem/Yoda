...
class PPTXConfig(LoaderConfig):
    ...
    Attributes:
        extract_notes: Whether to include speaker notes in the output.
        extract_tables: Whether to include table content from slides.
    """
...
class PPTXLoader(DocumentLoader):
    ...
    Supports ``SINGLE`` mode (all slides concatenated with headers) and
    ``PAGE`` mode (one ``LoadedDocument`` per non-empty slide).

    Args:
        config: PPTX-specific configuration. Defaults to ``PPTXConfig()``.
    """
    ...
    def _extract_slide_text(self, slide: Any) -> str:
        ...
        Returns:
            Concatenated text from shapes, tables, and speaker notes.
        """
        parts: list[str] = []
    ...
    def supports_source(self, source: str) -> bool:
        """Return ``True`` if *source* ends with ``.pptx``."""
