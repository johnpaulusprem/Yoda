...
class RecursiveChunkerConfig(ChunkerConfig):
    ...
    Attributes:
        separators: Ordered tuple of separator strings tried from highest
            to lowest priority (paragraph, line, sentence, exclamation,
            question, semicolon, comma, word).
        keep_separator: Whether to retain the separator at the start of
            each resulting piece.
        max_chunk_tokens: Maximum number of tokens (estimated) per chunk.
            Chunks exceeding this limit are split further. Uses the
...
class RecursiveChunker(BaseChunker):
    ...
    def chunk_document(self, document_id: str, text: str) -> list[Chunk]:
        ...
        # Step 5 -- validate token limits and split oversized chunks
        chunks = self._enforce_token_limit(chunks, document_id)
    ...
    def _estimate_tokens(self, text: str) -> int:
        ...
        For English text, 4 characters approximates 1 token with most
        modern tokenizers.  This avoids a hard dependency on ``tiktoken``
        while remaining accurate enough for budget checks.
    ...
    def _enforce_token_limit(
        ...
        """Split any chunk that exceeds ``max_chunk_tokens``.

        When a chunk is found to be over the token budget, it is re-split
        at the character limit implied by the token cap and sentence
        separators are preferred for the split.

        Args:
            chunks: Chunks to validate and possibly re-split.
            document_id: Document ID for metadata on newly created chunks.

        Returns:
            A list of chunks that all satisfy the token limit.
        """
        max_chars = self.config.max_chunk_tokens * 4
        validated: list[Chunk] = []

        for chunk in chunks:
            estimated = self._estimate_tokens(chunk.content)
            if estimated <= self.config.max_chunk_tokens:
                validated.append(chunk)
                continue

            # Re-split the oversized chunk using sentence separators
            logger.debug(
                "Chunk %s exceeds token limit (%d > %d), re-splitting",
            ...
            # Merge the sub-splits respecting the token-derived char limit
            sub_merged: list[str] = []
            current_buf: list[str] = []
            current_len = 0
            for piece in sub_splits:
                if current_len + len(piece) > max_chars and current_buf:
                    sub_merged.append("".join(current_buf))
                    current_buf = []
                    current_len = 0
            ...
            for sub_piece in sub_merged:
                meta = ChunkMetadata(
                    chunk_id=uuid.uuid4().hex,
                    document_id=document_id,
                    chunk_index=0,  # will be re-indexed by caller
                    start_char=chunk.metadata.start_char,
                    end_char=chunk.metadata.start_char + len(sub_piece),
                    token_count=self._estimate_tokens(sub_piece),
                )
                validated.append(Chunk(content=sub_piece, metadata=meta))
