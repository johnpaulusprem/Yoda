...
class ActionItemRepository(GenericRepository[ActionItem]):
    ...
    async def get_pending_for_nudge(self, cooldown_hours: int = 4) -> list[ActionItem]:
        ...
        result = await self._session.execute(select(ActionItem).where(ActionItem.status == "pending", ActionItem.deadline < datetime.now(UTC)).where((ActionItem.last_nudged_at == None) | (ActionItem.last_nudged_at < cutoff)))
        return list(result.scalars().all())
