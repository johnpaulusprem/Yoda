"""Background nudge scheduler for overdue action items in the meeting service.

Runs on an APScheduler interval (default 30 min). Each tick queries for
pending/in-progress action items whose deadline is within 24 hours or past,
respects a 4-hour per-item cooldown and active snoozes, then dispatches
nudge cards via DeliveryService. Items exceeding the escalation threshold
are escalated to the meeting organizer instead.
"""
