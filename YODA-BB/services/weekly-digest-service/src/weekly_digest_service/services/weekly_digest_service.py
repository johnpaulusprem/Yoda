...
from yoda_foundation.models import Meeting, ActionItem, MeetingSummary, MeetingParticipant
...
class WeeklyDigestService:
    ...
    async def generate_digest(self, user_id: str) -> WeeklyDigest:
        ...
        async with self._session_factory() as db:
            ...
            a_result = await db.execute(
                select(ActionItem).where(ActionItem.created_at >= week_start)
            )
            ...
            s_result = await db.execute(
                select(MeetingSummary).where(MeetingSummary.created_at >= week_start)
            )
            ...
            context = (
                f"Meetings: {len(meetings)}, Action items: {len(action_items)}, "
                f"Completed: {len(completed)}, Decisions: {len(all_decisions)}"
            )
            meeting_subjects = [m.subject for m in meetings[:10]]
            digest_text = await self._generate_digest_text(
                context, meeting_subjects, all_decisions[:10]
            )
    ...
    async def _generate_digest_text(
        self,
        context: str,
        meeting_subjects: list[str],
        decisions: list[Any],
    ) -> str:
        """Generate digest narrative text using AI, with fallback."""
        if not self._ai:
            return f"This week: {context}"
        ...
        try:
            ...
            digest_text = await self._ai.complete(
                ...
                messages=[
                    SystemMessage(
                        content=(
                            "Generate a concise weekly executive digest (3-4 paragraphs). "
                            "Focus on key outcomes, decisions, and upcoming priorities."
                        )
                    ),
                    UserMessage(
                        content=(
                            f"Week stats: {context}\n"
                            f"Meetings: {meeting_subjects}\n"
            ...
            logger.warning("AI digest generation failed, using fallback text")
            return f"This week: {context}"
    ...
    async def deliver_digest(self, digest_id: UUID) -> None:
        ...
        async with self._session_factory() as db:
            ...
            if digest and not digest.delivered:
                logger.info(
                    "Delivering weekly digest %s to %s", digest_id, digest.user_id
                )
