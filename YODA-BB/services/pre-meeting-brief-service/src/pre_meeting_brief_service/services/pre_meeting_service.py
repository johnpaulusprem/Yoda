...
from yoda_foundation.exceptions import AIProcessingError
...
class PreMeetingService:
    ...
    def __init__(
        ...
        Args:
            ...
            ai_connector: Optional AI connector for generating suggested
    ...
    async def generate_brief(
        ...
        attendee_task = self._gather_attendee_context(meeting, user_id)
        decisions_task = self._gather_past_decisions(meeting)
        documents_task = self._gather_related_documents(meeting)
        emails_task = self._gather_recent_emails(meeting, user_id)
        ...
        brief.suggested_questions = await self._generate_suggested_questions(brief)
    ...
    async def _gather_attendee_context(
        ...
        for participant in meeting.participants:
            ...
            if self.graph and participant.user_id:
                try:
                    user_info = await self.graph.get_user(participant.user_id)
    ...
    async def _gather_related_documents(
        ...
        try:
            files = await self.graph.get_recent_files(participant_ids)
    ...
    async def _gather_recent_emails(
        ...
        try:
            emails = await self.graph.get_user_emails(user_id, days=7)
    ...
    async def _generate_suggested_questions(
        ...
        try:
            ...
            raw = await self.ai_connector.complete(
                model="gpt-4o-mini",
                messages=[
