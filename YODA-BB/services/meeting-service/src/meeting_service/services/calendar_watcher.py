...
from yoda_foundation.schemas.webhook import GraphChangeNotification, GraphWebhookPayload
...
class CalendarWatcher:
    ...
    - Schedule APScheduler jobs so the Browser Bot joins before each meeting.
    """
    ...
    async def _process_notification(
        self, db: AsyncSession, notification: GraphChangeNotification
    ) -> None:
    ...
    async def _handle_created(
        self, db: AsyncSession, event: dict, user_id: str, join_url: str | None
    ) -> None:
    ...
    async def schedule_bot_join(self, meeting: Meeting) -> None:
        """Schedule the Browser Bot to join the meeting before its start time.
...
async def _execute_bot_join(meeting_id: str) -> None:
    """APScheduler job — tells the C# Browser Bot to join a meeting.
    ...
    from meeting_service.dependencies import async_session_factory
    ...
    async with async_session_factory() as db:
        ...
        from meeting_service.services.bot_commander import BotCommander, get_shared_bot_commander
