...
async def list_action_items(status: str | None = Query(None), assignee: str | None = Query(None), meeting_id: UUID | None = Query(None), limit: int = Query(20, ge=1, le=100), offset: int = Query(0, ge=0), db: AsyncSession = Depends(get_db), ctx: SecurityContext = Depends(get_current_user)):
...
@router.get("/overdue")
async def get_overdue(db: AsyncSession = Depends(get_db), ctx: SecurityContext = Depends(get_current_user)):
...
@router.get("/due-soon")
async def get_due_soon(hours: int = Query(48, ge=1), db: AsyncSession = Depends(get_db), ctx: SecurityContext = Depends(get_current_user)):
...
async def get_action_item(item_id: UUID, db: AsyncSession = Depends(get_db), ctx: SecurityContext = Depends(get_current_user)):
...
async def update_action_item(item_id: UUID, update: ActionItemUpdateRequest, db: AsyncSession = Depends(get_db), ctx: SecurityContext = Depends(get_current_user)):
